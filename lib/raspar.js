 ...
 * @param {Integer} [options.ttl] TTL (Time to live) in seconds for local/memory based cache. Default: 1800
...
const requestFromUrl = (url, options) => {
    ...
    const settings = {...OPTION_DEFAULTS,
        ...options};
    ...
    return request({url,
        ...settings.requestOptions}).then(res => {
...
const formatCache = res => ({...JSON.parse(res),
    'cached': true});

/**
 * Returns cache from memory based on a specific hash.
 *
 * @param {String} hash Hash generated based on the URL.
 * @return {Object} Promise
 * @private
 */

const requestFromCacheMemory = hash =>
    getMemoryCache(hash).then(res => {
 ...
 * @param {String} settings.cacheDirectory Directory to store cache.
 * @param {Integer} settings.ttl TTL (Time to live) in seconds for local/memory based cache.
 * @return {Object} Promise
 * @private
 */

const requestFromCacheDirectory = (hash, settings) => {

    const cachePath = path.join(settings.cacheDirectory, hash);

    return stat(cachePath).then(stats => {

        if (
            stats &&
            (Date.now() - stats.mtime.getTime()) / milliseconds < settings.ttl
        ) {
...
const requestFromCache = (url, options) => {
    ...
    if (settings.cacheMemory) {
        ...
        return requestFromCacheMemory(hash);

    } else if (settings.cacheDirectory) {

        return requestFromCacheDirectory(hash, settings);
...
const requestFromUrlorCache = (url, options) =>
    requestFromCache(url, options).catch(() => requestFromUrl(url, options));

/**
 * Requests a URL or an array of URLs.
 *
 * @example raspar.fetch('http://www.google.com/humans.txt').then(res => {});
 * @example raspar.fetch(['http://www.google.com/humans.txt']).then(res => {});
